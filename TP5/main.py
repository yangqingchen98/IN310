...
class BidBehaviour(Behaviour):
    ...
    def execute(self, message):
        ...
        if message.performative == ACLMessage.CFP:
            good_cfp = Good.from_string(message.content)
            proposal = ACLMessage(ACLMessage.PROPOSE)
            proposal.add_receiver(message.sender)
            proposal.set_content(str(self.bid(good_cfp)))
            self.agent.send(proposal)
            display_message(self.agent.aid.name, 'CFP received from: {}'.format(message.sender))
        ...
        # - Compute new  utility of the agent: add the overall_utility of the good contained in the message and remove the value in the content
            ...
            value_accept, good_accept = message.content.split(",")
            self.agent.remaining_budget -= float(value_accept)
            self.agent.overall_utility += utility(Good.from_string(good_accept), self.agent) - float(value_accept)
            display_message(self.agent.aid.name, 'Accept proposal from: {}'.format(message.sender))
